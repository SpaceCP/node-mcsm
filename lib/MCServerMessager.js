...
*/
var MCServerMessger = function(options) {
...
function onConnect(con, that) {
	...
	con.on("data", function(buf) {
		...
		if(type == 0x02) {	//Login request, respond with info message
		...
		} else if(type == 0xFE) {	//Server status request, send MOTD and player ratio info
			...
			*/
				...
				buf.writeUInt16LE(Math.floor((len - 4)/ 2), 2);	//packet payload lngth

				buf.writeUInt8(0xA7, 4);
				buf.writeUInt8(0x00, 5);
				buf.writeUInt8("1".charCodeAt(0), 6);	//motd version
				...
				bufCpy(tmp, offset, that.options.protocolVersion.length*2, buf);
				...
				bufCpy(tmp, offset, that.options.reportedVersion.length*2, buf);
				...
				bufCpy(tmp, offset, that.options.motd.length*2, buf);
				...
				bufCpy(tmp, offset, that.options.players.length*2, buf);
				...
				bufCpy(tmp, offset, that.options.maxPlayers.length*2, buf);
			
				buf.writeUInt8(0x00, offset + that.options.maxPlayers.length*2);
				buf.writeUInt8(0x00, offset + that.options.maxPlayers.length*2 + 1);
...
module.exports = MCServerMessger;