			...
			if(packetId == 0x00) {	//handshake packet
				...
				if(packetLength == 1 && that.clientState == clientStates.status) {	//Handshake STATUS REQUEST packet
					...
					*/
					...
					if(nextState == 0x01) {	//NextState => STATUS
					...
					} else if(nextState == 0x02) {	//nextState => LOGIN
				...
				packet.writeUInt32BE(HTime, offset);
				offset += 4;

				packet.writeUInt32BE(LTime, offset);
